...
import { runMmaTemplateService } from '../../../../../lib/music-services/mma-service';
...
export async function POST(request: Request) {
  ...
  try {
    const result = await runMmaTemplateService(await request.json(), { traceContext: trace });
    ...
    const message = error instanceof Error ? error.message : 'MMA template generation error.';
    status = 500;
    return tracedJson({ error: message }, { status });
  } finally {
    logApiRouteSummary({
      event: 'music.mma.template.summary',
      route: '/api/music/mma/template',
      method: 'POST',
      status,
      startedAt,
      trace,
